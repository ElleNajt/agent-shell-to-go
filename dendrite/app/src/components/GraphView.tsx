...
export function GraphView({
    ...
    // Use controlled viewport if provided, otherwise local state
    ...
    // Track current offset in a ref for panResponder access
    const currentOffsetRef = useRef(offset);
    currentOffsetRef.current = offset;

    // Track the offset at the start of a drag gesture (set in onGrant)
    ...
    // Store callbacks in refs so panResponder can access latest versions
    const onViewportChangeRef = useRef(onViewportChange);
    const setLocalOffsetRef = useRef(setLocalOffset);
    const setLocalScaleRef = useRef(setLocalScale);
    onViewportChangeRef.current = onViewportChange;
    setLocalOffsetRef.current = setLocalOffset;
    setLocalScaleRef.current = setLocalScale;
    ...
    const panResponder = useRef(
        PanResponder.create({
            ...
            onPanResponderGrant: (evt: GestureResponderEvent) => {
                ...
                // Capture current offset at start of gesture
            ...
            onPanResponderMove: (evt: GestureResponderEvent, gestureState) => {
                ...
                if (
                    ...
                    const newScale = Math.max(
                        0.3,
                        Math.min(3, scaleRef.current * pinchScale),
                    );
                    // Update scale via callback or local state
                    if (onViewportChangeRef.current) {
                        ...
                        setLocalScaleRef.current(newScale);
                    }
                ...
                if (!isPinching.current) {
                    ...
                    if (onViewportChangeRef.current) {
                        ...
                        setLocalOffsetRef.current(newOffset);
                    }
                }
            },
            onPanResponderRelease: () => {
                ...
                // Pan offset is already committed via state updates during move
            },
        }),
    ).current;
    ...
    const { nodes, edges } = useMemo(() => {
        ...
        agents.forEach((agent) => {
            ...
            if (
                agent.project === ".claude-meta" ||
                agent.project.endsWith("/.claude-meta")
            ) {
                metaAgent = agent;
            }
        });
        ...
        agents.forEach((agent) => {
            ...
            if (parentId) {
                ...
                const isMetaAgent =
                    agent.project === ".claude-meta" ||
                    agent.project.endsWith("/.claude-meta");
                const isDispatcher = agent.buffer_name
                    .toLowerCase()
                    .includes("dispatcher");
        ...
        if (nodesByDepth[0]) {
            ...
            nodesByDepth[0].forEach((agent, i) => {
                ...
                if (radius === 0) {
                    ...
                    angle =
                        (2 * Math.PI * i) / nodesByDepth[0].length -
                        Math.PI / 2;
        ...
        if (nodesByDepth[1]) {
            ...
            sortedDispatchers.forEach((agent, i) => {
                const angle =
                    (2 * Math.PI * i) / sortedDispatchers.length - Math.PI / 2;
            ...
            projects.forEach((project) => {
                ...
                projectAgents.forEach((agent, i) => {
                    ...
                    if (dispatcherAngle !== undefined) {
                        ...
                        angle =
                            (2 * Math.PI * agentIndex) / totalAgents -
                            Math.PI / 2;
                    }
    ...
    return (
        <View style={styles.container} {...panResponder.panHandlers}>
            ...
            >
                ...
                {nodes.map((node) => {
                    const isSelected =
                        selectedAgent?.session_id === node.agent.session_id;
                    const statusColor =
                        STATUS_COLORS[node.agent.status] || STATUS_COLORS.ready;
                    const isDispatcher = node.agent.buffer_name
                        .toLowerCase()
                        .includes("dispatcher");
                    const name =
                        node.agent.buffer_name.split(" @ ")[0] ||
                        node.agent.buffer_name;
                    const projectName =
                        node.agent.project.split("/").pop() ||
                        node.agent.project;
                    const isUnread =
                        hasUnread?.(
                            node.agent.session_id,
                            node.agent.last_activity,
                        ...
                        >
                            {isDispatcher ? (
                                <>
                                    ...
                                    <Text style={styles.dispatcherProject}>
                                        {projectName}
                                    </Text>
                                    <View
                                        style={[
            ...
            {agents.length === 0 && (
                <View style={styles.empty}>
                    ...
                    <Text style={styles.emptyHint}>
                        Start an agent in Emacs to see it here
                    </Text>
                </View>
            )}
